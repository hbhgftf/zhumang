	...
	export default {
		...
		methods: {
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.statusCode === 200 && res.data.code === 200) {
							...
							console.log('[TUICallKit] login 参数:', loginParams);
							
							console.log('[被叫方TUICallKit] login 参数:', { SDKAppID, userID: this.userID, userSig });
							uni.$TUICallKit.login({ SDKAppID, userID: this.userID, userSig }, res => {
								console.log('[被叫方TUICallKit] login 回调:', res, 'userID:', this.userID, 'userSig:', this.tuicallkitUserSig, 'SDKAppID:', this.tuicallkitSDKAppID);
								if (res.code === 0) {
									this.isLogin = true;
			...
			handleCallAccepted(message) {
				console.log('[WebSocket] 收到 CALL_ACCEPT:', message);
				const userIDList = [String(message.calleeId)];
				console.log('[handleCallAccepted] userIDList:', userIDList, '类型:', typeof userIDList[0], 'userID:', this.userID, 'userSig:', this.tuicallkitUserSig, 'SDKAppID:', this.tuicallkitSDKAppID);
				// 作为主叫收到接受通知，开始通话
			...
			startCall(roomId, calleeId) {
				try {
					const userIDList = [String(calleeId)];
					const callParams = {
					...
					console.log('[startCall] userIDList:', userIDList, '类型:', typeof userIDList[0], 'userID:', this.userID, 'userSig:', this.tuicallkitUserSig, 'SDKAppID:', this.tuicallkitSDKAppID);
					console.log('[TUICallKit] 调用 calls 参数:', callParams);

					uni.$TUICallKit.calls(callParams, res => {
						console.log('[TUICallKit] calls 返回:', res);
						if (res.code !== 0) {
							this.isRequesting = false;
							uni.showToast({
