...
export default {
  ...
  methods: {
    ...
    async handleAcceptCall() {
      ...
      // 日志：打印 callRequestInfo
      console.log('[handleAcceptCall] callRequestInfo:', this.callRequestInfo);
      if (!this.callRequestInfo.roomId) {
        console.error('[handleAcceptCall] roomId 为空，无法发起通话');
        uni.showToast({ title: '通话房间信息缺失', icon: 'none' });
        return;
      }
      if (!this.callRequestInfo.callerId) {
      ...
      try {
        ...
        if (res.data.code === 200) {
          this.isInCall = true
          console.log('[handleAcceptCall] 接受通话成功，准备调用 answerCall，roomId:', this.callRequestInfo.roomId, 'callerId:', this.callRequestInfo.callerId);
          // 使用TUICallKit接听通话
    ...
    answerCall(roomId, callerId) {
      ...
      console.log('[answerCall] 入参 roomId:', roomId, 'callerId:', callerId);
      if (!callerId) {
        console.error('[TUICallKit] answerCall: callerId 为空，无法发起通话');
        uni.showToast({ title: '通话发起人信息缺失', icon: 'none' });
        return;
      }
      if (!roomId) {
        console.error('[TUICallKit] answerCall: roomId 为空，无法发起通话');
        uni.showToast({ title: '通话房间信息缺失', icon: 'none' });
        return;
      }
      try {
        ...
        if (!isNaN(roomID) && String(roomID) === roomId) {
          callParams.callParams.roomID = roomID;
        }
        console.log('[answerCall] 调用 TUICallKit.calls 参数:', callParams, 'roomID类型:', typeof callParams.callParams.roomID, 'roomID值:', callParams.callParams.roomID);
    ...
    async loginToTUICallKit() {
      ...
      try {
        const res = await uni.request({
          url: `${this.$config.baseUrl}/user/userSig`,
          method: 'GET',
          data: { userID }
        });
        ...
        if ((res[1] && res[1].statusCode === 200 && data.code === 200) || (res.statusCode === 200 && data.code === 200)) {
          ...
          console.log('[被叫方TUICallKit] login 参数:', { SDKAppID, userID, userSig });
          uni.$TUICallKit.login({ SDKAppID, userID, userSig }, loginRes => {
            console.log('[被叫方TUICallKit] login 回调:', loginRes, 'userID:', userID, 'userSig:', userSig, 'SDKAppID:', SDKAppID);
            if (loginRes.code === 0) {
              this.isLogin = true;
              console.log('[被叫方TUICallKit] login success');
            } else {
              this.isLogin = false;
              uni.showToast({ title: '视频通话系统登录失败', icon: 'none' });
            }
          });
        } else {
          this.isLogin = false;
